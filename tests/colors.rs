...
fn osc_color_overrides_and_resets_are_engine_state() {
    ...
    assert_eq!(colors.ansi[1], Some(RgbColor { r: 0x12, g: 0x34, b: 0x56 }));
    assert_eq!(colors.foreground, Some(RgbColor { r: 0xab, g: 0xcd, b: 0xef }));
    assert_eq!(colors.background, Some(RgbColor { r: 0x22, g: 0x33, b: 0x44 }));
    assert_eq!(colors.cursor, Some(RgbColor { r: 0x65, g: 0x43, b: 0x21 }));
